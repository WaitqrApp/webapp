...
function TableModal({ mesas }) {
  ...
  const { sesiongeneralmesa, obtenerSesionGeneral, eliminarSesionGeneral } =
    sesionGeneralsContext;

  const sesionIndividualsContext = useContext(sesionIndividualContext);
  const { sesionindividualsesiongeneral, obtenerSesionIndividual } =
  ...
  const cerrarMesa = (e) => {
    eliminarSesionGeneral(sesiongeneralmesa[0]._id, sesiongeneralmesa[0].mesa);
    setShow(false);
  };
