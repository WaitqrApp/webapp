...
export default (state, action) => {
  switch (action.type) {
    case PLATILLOORDENADO_ORDEN:
      return {
        ...
        platilloOrdenadoOrden: [
          ...state.platilloOrdenadoOrden.filter(
            ({ _id }) => !action.payload.map((po) => po._id).includes(_id)
          ),
          ...action.payload,
        ],
      };
    ...
    case ELIMINAR_PLATILLOORDENADO:
      return {
        ...state,
        platilloOrdenadoOrden: state.platilloOrdenadoOrden.filter(
          (platilloOrdenado) => platilloOrdenado._id !== action.payload
        ),
      };
    ...
    case PLATILLOORDENADO_ACTUAL:
      return {
        ...state,
        orden: state.platilloOrdenadoOrden.filter(
          (platilloOrdenado) => platilloOrdenado._id === action.payload
        ),
      };
