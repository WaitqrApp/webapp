...
export default (state, action) => {
    switch(action.type){
        ...
        case ELIMINAR_MENU:
            return{
                ...state,
                menusrestaurante: state.menusrestaurante.filter(menu => menu._id !== action.payload)
            }
        case ACTUALIZAR_MENU:
            return{
                ...state,
                menusrestaurante: state.menusrestaurante.map(menu => menu._id === action.payload._id 
                    ? action.payload
                    : menu)
            }
        case MENU_ACTUAL:
